...
function matchesFilter(q, sourceFilter) {
  ...
  if (["nutrition", "exercises", "muscles", "equipment", "movements"].includes(sourceFilter)) return q.source === sourceFilter;
...
function runSession(questions, sourceFilter, storage, random) {
  ...
  if (sourceFilter === "mixed") {
    ...
    for (const category of ["nasm", "nsca", "nutrition", "exercises", "muscles", "equipment", "movements"]) {
...
const filters = ["nasm", "nsca", "nutrition", "exercises", "muscles", "equipment", "movements", "mixed"];
let seed = 0.42;
const random = () => {
  seed = (seed * 16807) % 1;
  return seed;
